...
## 3. 目录结构（当前实际）
    ...
    ├── docs/                           # 数据库设计说明书（docx / ER 图 / H2 与 MySQL DDL），由 scripts/gen_db_design_doc.py 生成
...
32. **用户管理的不变量（改 `/api/users` 或「用户管理」页时必须守住）**：① 端点保持**类级** `@PreAuthorize("hasRole('ADMIN')")`（前端入口与视图同样只对 ADMIN 展示，但真正的边界在后端）；② **不能删除或禁用当前登录账号**（`delete(id, currentUserId)`、`batchUpdateStatus` 对 self 单独拦截；前端勾选列 `:selectable="row => !isSelf(row)"` 与删除按钮 `:disabled` 同步挡住）；③ **必须保留至少一个启用管理员**（`protectLastAdmin`，删除 / 降级 / 禁用三条路径都要过，批量同样逐项过）；④ 重置密码或禁用账号后必须让旧登录态失效（`bumpTokenVersion`，与红线 24 一致）；⑤ 所有写操作写审计（单条 USER_CREATE / USER_UPDATE / USER_PASSWORD_RESET / USER_DELETE；批量 USER_BATCH_STATUS / USER_BATCH_DELETE）。**批量实现要点**：走独立端点 `POST /api/users/batch-status|batch-delete`（与作业 `batch-online/offline` 同范式），服务层 `runBatch` **逐项独立事务**——批量方法**不要加外层 `@Transactional`**，否则内层抛异常会把外层标记 rollback-only，捕获后仍整体回滚。改完跑 `node scripts/verify-users-crud.cjs`（真浏览器 12 项断言）+ `frontend/test/users-admin.test.js` + `UserServiceTest`。
33. **in-DOM 模板里不要用会与原生标签冲突的图标名**：本项目 `index.html` 是浏览器**先解析**的 in-DOM 模板，`<el-icon><select /></el-icon>` 会被当成**原生 `<select>`**，把后续模板内容吞进去、整块界面结构被破坏（本轮真实踩坑：批量启用按钮写了 `<select />`，导致新建/编辑对话框全部点不开，12 项端到端断言里 8 项挂掉）。请用 `<circle-check />`、`<close />`、`<delete />`、`<key />` 这类与原生标签不冲突的名字；按钮内必须带图标（`frontend/test/app.test.js` 有强制用例）。
...
### 9.4 自动化测试（改完必跑，2026-09-18 实测全绿）
- [ ] `mvn -B -f pom.xml test` → 5 模块 BUILD SUCCESS，后端 **170** 个用例 0 失败。
- [ ] `cd frontend; node --test test/*.test.js` → **66** 个用例 0 失败（控件注册表一致性、控件管理页、按钮图标、数据源 CRUD、账户资料与在线状态、参数类型转换、模板与 AI 示例、端点权限入口隐藏、资源生命周期、拖拽落点幂等、参数持久化、用户管理界面与批量操作接线）。
- [ ] 改了用户/权限相关代码时跑一次真浏览器验证：`node scripts/verify-users-crud.cjs`
      → **12 项断言全 PASS**（管理菜单入口、列表、新建、编辑、重置密码、删除、自删保护、审计四类记录、勾选计数、批量禁用、批量启用、批量删除 + 批量审计；脚本会自行删除测试账号）。
- [ ] 改了任意页面布局时可截图核对：`node scripts/shot-page.cjs "控件管理" out.png`（通用：菜单项名 + 输出路径，
      会打印视图类名 / 标题 / 统计行 / 表格行数 / 只读提示，便于无人值守确认排版）。
...
| 2026-09-17 | **产品更名：数据流任务管理系统 → 通用流处理任务管理平台**：按需求把产品名在**整个工作区**统一替换（同时收编旧变体「数据流任务管理平台」），共 **18 个文件 30 处**——前端 `index.html`（浏览器标题 / 顶栏 h1 / 帮助中心「关于」）、`README.md`、`DEVELOPMENT.md`、`backend/pom.xml` 描述、`data/mysql_setup.sql`、`test-resources/*`（system_config.json、benchmark.py、run-benchmark.bat）、`test-results/benchmark_report.md`、`gen_test_data.py`、`scripts/test-mysql-flow.ps1`、`docs/作品演示录像脚本.md`、`scripts/*`（视频卡片 / 成片 / DB 说明书生成器）、`frontend/public/css/style.css`。替换用 Python 逐文件执行并**保留原 BOM 与换行**（5 个文件原带 BOM），规避 javac `\ufeff` 与 PowerShell 编码坑。**同步重生成**一切内嵌旧名的产物：视频卡片（片头/片尾）、成片 `作品演示录像_5min.mp4`、`旁白文稿.md/.docx`、`docs/数据库设计说明书.docx` + `docs/数据库-ER图.png`。回归：前端 **49 用例 0 失败**、`node --check app.js` 通过、三个 Python 生成器语法通过；工作区已无旧名残留（`.claude/worktrees/*` 是并行会话的工作副本，刻意未动） | index.html · README.md · DEVELOPMENT.md · backend/pom.xml · data/mysql_setup.sql · test-resources/* · test-results/benchmark_report.md · gen_test_data.py · scripts/* · docs/* || 2026-09-17 | **修掉演示卡片「字出框」**：性能测试卡底部评审要点框（原 300–1620）内文字按固定字号 30 直接绘制，长句越过框线。改为统一排版层：新增 `fit_font/center_fit/left_fit`，**按实测文本宽度逐磅缩小字号**（下限 15pt），框加宽到与其它卡一致的 140–1780 并留 40pt 内边距；同时加**排版自检**——任何一行越过 60pt 安全边距即打印 `[overflow]` 并以退出码 1 失败，避免以后再漏。四张卡（片头/架构/性能/片尾）全部改为自适应排版，自检通过、抽帧确认；成片 `作品演示录像_5min.mp4` 已重建并同步桌面交付目录 | scripts/make_video_cards.py · output/_video_work/cards/* + 成片 || 2026-09-18 | **补齐「用户管理」界面（增删查改）**：后端 `/api/users` 自 2026-09-08 起已具备完整 CRUD（类级 ADMIN 限制 + 审计 + 防删当前账号 + 保留至少一个启用管理员 + 重置密码吊销 token），但**前端一直没有入口与界面**（顶栏「管理」下只有操作审计与运营总览），管理端实际做不到增删用户。本次补齐：① 顶栏「管理 → 用户管理」（仅 ADMIN 可见）；② 新视图 `users-view`——搜索用户名/显示名、按角色筛选、前端分页列表（ID/用户名/显示名/角色/状态/创建时间），角色与状态用彩色 tag，当前登录账号带「当前登录」标记；③ 四个操作：**新建**（用户名 3–32 位字母数字下划线、显示名必填、密码 ≥8 位且含字母和数字，角色默认 VIEWER）、**编辑**（显示名/角色/启用开关，用户名不可改）、**重置密码**（同一强度校验，提示旧登录态失效）、**删除**（ElMessageBox 二次确认）；④ 保护措施在界面上同步体现——本人删除按钮 `:disabled`、非 ADMIN 进入即弹回工作台并提示、表头展示「启用中的管理员 N 个」。**验证**：新增真浏览器端到端脚本 `scripts/verify-users-crud.cjs`（Edge + CDP：登录 → 管理菜单 → 列表 → 新建 → 编辑 → 重置密码 → 删除 → 自删保护 → 审计四类记录，脚本自清理测试账号）**7/7 PASS**；新增前端静态回归 `frontend/test/users-admin.test.js`（7 例：入口权限、视图与四操作、API 五方法、权限兜底、自删保护、密码强度、模板变量暴露），前端 **56** 用例 0 失败；后端 **166** 用例 0 失败（`UserServiceTest` 13 例已覆盖删除/降级/重置密码三处保护）。文档：README 增「用户管理」功能条目与 `/api/users` 接口表、§8 红线新增第 32 条、§9.4 加入验证命令 | frontend/public/index.html · js/app.js · frontend/test/users-admin.test.js（新）· scripts/verify-users-crud.cjs（新）· README.md · DEVELOPMENT.md || 2026-09-18 | **用户管理加批量操作（启用 / 禁用 / 删除）**：在上一轮补齐的「用户管理」界面上增加勾选列（`:selectable` 排除当前登录账号）与批量操作栏（批量启用 / 批量禁用 / 批量删除 / 清空选择，未勾选时禁用），批量前二次确认（删除弹窗会列出将被删除的账号名）。后端新增 `POST /api/users/batch-status`、`POST /api/users/batch-delete`（与作业 `batch-online/offline` 同范式），服务层 `runBatch` **逐项独立事务**：单项失败（当前登录账号、最后一个启用管理员、用户不存在）只记入 `failed` 并附带原因，不影响其它项，返回 `{succeeded, failed:[{id,message}], total}`；前端按「成功 N，失败 M + 逐项原因」提示（把 id 映射回用户名）。**刻意不给批量方法加外层 `@Transactional`**——内层异常会把外层标记 rollback-only，捕获后仍会整体回滚。**踩坑记录**：批量启用按钮起初用了 `<el-icon><select /></el-icon>`，而 `index.html` 是浏览器先解析的 in-DOM 模板，`<select>` 是原生标签 → 后续模板被吞进 select、新建/编辑对话框全部点不开（端到端 12 项里 8 项挂），换 `<circle-check />` 后恢复；已写入红线 33。验证：`scripts/verify-users-crud.cjs` **12/12 PASS**（新增勾选计数、批量禁用/启用/删除、批量审计四项）；`UserServiceTest` +4 例（自删被拒但他人成功、最后一个管理员保护、禁用自己被拒、空选择 400）→ 后端 **170** 用例 0 失败；前端静态回归 +4 例 → **60** 用例 0 失败。文档：README 功能条目与 `/api/users` 表补批量端点、§8 新增红线 33、§9.4 更新计数与断言数 | UserService · UserController · UserServiceTest · index.html · js/app.js · frontend/test/users-admin.test.js · scripts/verify-users-crud.cjs · README · DEVELOPMENT || 2026-09-18 | **修用户管理页两处界面缺陷（用户截图反馈）**：① 角色筛选下拉**缺「全部角色」选项**（原来只有 ADMIN/OPERATOR/VIEWER，靠 placeholder 与 clearable 的 × 表示"全部"，不清不楚）→ 补 `<el-option label="全部角色" value="">` 作为首项；② **批量按钮被网格挤到换行并被裁掉**（截图里只看得到「批量删除」）→ 根因是批量元素被塞进 `.audit-filter`，而它是 `grid-template-columns:repeat(6,...)` 的六列网格；改为按作业页范式**把批量栏独立成 `.users-batch-bar` 一行**（flex + 背景条 + 右侧提示「勾选后即可批量操作；当前登录账号不可勾选」），并在 style.css 新增对应样式；③ 顺手把「创建时间」列改用既有 `fmtDateTime()`（后端 ISO 带 6 位微秒会折行，现在显示 `2026-08-16 12:08`）——**注意项目里已有 `fmtDateTime`，不要再定义第二个**（本轮踩坑：重复声明直接让整份 app.js 语法错误、页面全白）。验证：新增 `scripts/shot-users-page.cjs`（CDP 截图 + 断言下拉选项与批量栏按钮），截图目视确认「全部角色」在列、四个批量按钮全部可见、时间不换行；前端静态回归 **62** 用例 0 失败（新增布局与选项断言，含"批量按钮不得塞进筛选卡片"的回归）；端到端 `verify-users-crud.cjs` 仍 **12/12 PASS** | index.html · style.css · js/app.js · frontend/test/users-admin.test.js · scripts/shot-users-page.cjs（新）/ DEVELOPMENT 变更记录 || 2026-09-18 | **按需求移除仓库内的演示视频相关内容**：① 删除 Release `v1.0-demo`（成片 174MB / 10MB 两个视频 + 字幕 + 旁白文稿，附件不在 git 内，删 Release 即彻底消失）并删除残留远端标签 `v1.0-demo`；② 从仓库移除 4 个纯演示视频工具脚本（`scripts/video_tool.py`、`make_video_cards.py`、`build_demo_video.py`、`make_narration_script.py`，本地副本保留在桌面 `演示成片_输出/tools/`）；③ 清理 README「相关文档」中指向已删除 `docs/` 内容的失效条目（演示录像脚本、演示材料、数据库设计说明书三组引用，见下条）；④ DEVELOPMENT §6 文件树 `docs/` 注释同步更新。**注意**：并行的 `1e08d85 "Delete docs directory"` 把整个 `docs/` 目录删除，连带删掉了**数据库设计说明书.docx / 数据库-ER图.png / schema-h2.sql / schema-mysql.sql**（与视频无关的参赛交付物），是否恢复另行确认 | scripts/*（删除 4 个视频脚本）· README §十一 · DEVELOPMENT §6 | | 2026-09-18 | **恢复「控件管理」页并按需求上传**（2026-09-16 曾以 A 方案摘入口下线）：导航恢复「开发资源 → 控件管理」入口与 `controls-view` 视图，并在恢复时增强（原版是纯表格）——头部**分类统计**（共 N 个 · 输入/转换/输出 · 内置 + 插件 · 当前启用）、**关键词搜索**（type/name/description）与**分类筛选**（含「全部分类」）、**来源列区分内置/插件**（插件显示 jar 名）、表头下方**只读说明条**（交代注册表由 `DataInitializer` 启动 upsert 维护、插件 jar 放 `backend/plugins/` 重启即注册、在线编辑不生效故不做），`el-switch` 保持 `disabled`。切换视图会 `loadControls()` 刷新，`loadControls` 增加 `controlsLoading` 状态。测试：`frontend/test/controls-page.test.js` 从「防止加回来」反转为 6 例守页（入口/视图/七列/只读语义/统计筛选/切视图刷新/注册表消费链/setup 暴露），前端 **66** 用例 0 失败。验证：通用截图脚本 `scripts/shot-page.cjs`（取代 `shot-users-page.cjs`，可指定任意菜单项）实拍确认——视图 `controls-view`、标题「🧩 控件管理」、统计「共 31 个（输入 11 / 转换 10 / 输出 10）· 内置 31 + 插件 0 · 当前启用 31」、表格 31 行。文档：README §四 补查看入口说明、§8 红线 28 更新为「三个展示入口 + 不要做成在线编辑」、§9.4 更新计数并加入截图脚本用法 | index.html · js/app.js · css/style.css · frontend/test/controls-page.test.js · scripts/shot-page.cjs（新，替换 shot-users-page.cjs）· README · DEVELOPMENT |