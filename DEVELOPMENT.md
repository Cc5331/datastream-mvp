...
28. **控件注册表是「种子数据」，不要再做在线编辑入口**：`DataInitializer` 启动时 `deleteAll()` 后只重建内置控件，且插件加载器是 `@PostConstruct`（早于 `CommandLineRunner`），所以通过 API / 插件写进去的控件重启后会消失；翻译层又按 `node.getType()` 硬编码分支，新增一行控件 ≠ 能跑。原「控件管理」页（纯只读表格、与画布控件库和帮助中心重复）已于 2026-09-16 下线，控件展示统一走**画布左侧控件库**与**帮助中心「控件说明」**。将来若要做插件化管理，必须先修「插件控件被清空」的启动顺序问题并补回归用例。
