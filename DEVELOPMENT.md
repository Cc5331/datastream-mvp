...
- [ ] `cd frontend; node --test test/*.test.js` → **45** 个用例 0 失败（控件注册表一致性、按钮图标、数据源 CRUD、账户资料与在线状态、参数类型转换、模板与 AI 示例、端点权限入口隐藏、资源生命周期、拖拽落点幂等）。
- [ ] 改了画布交互（拖拽/连线/加载 DAG）时跑一次真浏览器验证：`node scripts/verify-drag-single-node.cjs`
      → 输出 `✅ PASS：一次拖拽只产生一个控件`（需要 3000/18080 已启动 + Edge；可跑的前提是 `ws` 模块可解析，必要时用 `DSH_WS_NODE_MODULES` 指定）。
