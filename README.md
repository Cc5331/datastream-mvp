...
- **用户管理（增删查改 + 批量操作，仅 ADMIN）**：顶栏「管理 → 用户管理」可视化维护账号——搜索用户名/显示名、按角色筛选、分页列表，支持**新建**（用户名 3–32 位字母/数字/下划线，密码 ≥8 位且同时含字母和数字）、**编辑**显示名与角色、**启用/禁用**、**重置密码**（重置后该账号旧登录态立即失效）、**删除**（二次确认）；勾选多行后可**批量启用 / 批量禁用 / 批量删除**，逐项独立处理并回报「成功 N，失败 M + 原因」。后端 `/api/users` 为类级 ADMIN 限制：禁止删除或禁用**当前登录账号**、禁止删掉或降级**最后一个启用管理员**，所有写操作全部写入审计（USER_CREATE / USER_UPDATE / USER_PASSWORD_RESET / USER_DELETE / USER_BATCH_STATUS / USER_BATCH_DELETE）。
...
> 查看入口：顶栏「**开发资源 → 控件管理**」是控件注册表的只读视图（分类统计、关键词搜索、分类筛选、内置/插件来源标识）；
> 画布左侧控件库按分类提供拖拽，帮助中心「控件说明」提供同类清单。注册表由后端启动种子与插件加载器维护，**页面不提供在线编辑**——新增/修改控件请走代码或插件 jar（详见 §九）。
...
### 用户管理（/api/users，类级仅 ADMIN，写操作全部审计）

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/users | 用户列表（id / 用户名 / 显示名 / 角色 / 是否启用 / 创建时间） |
| POST | /api/users | 新建用户（用户名 3–32 位字母数字下划线、密码 8–128 位、角色默认 VIEWER、默认启用） |
| PUT | /api/users/{id} | 修改显示名 / 角色 / 启用状态（不允许取消最后一个启用管理员） |
| PUT | /api/users/{id}/password | 管理员重置密码（旧 token 立即失效，被重置者可感知需重新登录） |
| DELETE | /api/users/{id} | 删除用户（不能删除当前登录账号；不允许删掉最后一个启用管理员） |
| POST | /api/users/batch-status | 批量启用/禁用（body `{ids:[], enabled:true|false}`），返回 `{succeeded:[], failed:[{id,message}], total}` |
| POST | /api/users/batch-delete | 批量删除（body `{ids:[]}`），返回同上；逐项独立处理，单项失败不影响其它项 |

> 前端入口：顶栏「管理 → 用户管理」（仅 ADMIN 可见）。端到端回归：`node scripts/verify-users-crud.cjs`（真浏览器走完增删改查 + 自删保护 + 审计校验，脚本自清理测试账号）。
