...
- **用户管理（增删查改，仅 ADMIN）**：顶栏「管理 → 用户管理」可视化维护账号——搜索用户名/显示名、按角色筛选、分页列表，支持**新建**（用户名 3–32 位字母/数字/下划线，密码 ≥8 位且同时含字母和数字）、**编辑**显示名与角色、**启用/禁用**、**重置密码**（重置后该账号旧登录态立即失效）、**删除**（二次确认）。后端 `/api/users` 为类级 ADMIN 限制：禁止删除当前登录账号、禁止删掉或降级**最后一个启用管理员**，四类操作全部写入审计（USER_CREATE / USER_UPDATE / USER_PASSWORD_RESET / USER_DELETE）。
